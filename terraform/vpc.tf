...
############################################################
# VPCエンドポイント用セキュリティグループ（Interface 用）
############################################################
resource "aws_security_group" "vpce" {
  ...
  # VPC内部（10.0.0.0/16）からのHTTPS（443）アクセスを許可
  ingress {
    from_port   = 443
    to_port     = 443
    protocol    = "tcp"
...
# --- SSM関連（EC2のSSM接続 & Session Manager用）---
...
# --- ECR用（Fargate がコンテナイメージをPullするために必須）---
